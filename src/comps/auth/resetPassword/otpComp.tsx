...
interface OTPInputProps {
    ...
    verifyOTP: (params: { email: string; otp: string }) => Promise<{ success: boolean; message?: string; token?: string }>;
    resendOTP: (params: { email: string }) => Promise<{ success: boolean; message?: string }>;
    token?: string;
    rememberMe?: boolean;
}

const OTPInput = ({ email, onVerificationComplete, verifyOTP, resendOTP }: OTPInputProps) => {
    ...
    const handleVerify = async (): Promise<void> => {
        ...
        try {
            ...
            // Pass back the token for the reset password flow
            onVerificationComplete(result.success, result.token);
        } catch (err) {
            setIsVerifying(false);
    ...
    const handleResend = async (): Promise<void> => {
        ...
        try {
            ...
            if (result.success) {
                showNotification("OTP Sent", "New OTP has been sent to your email", "success");
            } else {
                setError(result.message || 'Failed to resend OTP');
                showNotification("Error", result.message || "Failed to resend OTP", "error");
            }
        } catch (err) {
            setError('Resend error');
            showNotification("Error", "Failed to resend OTP", "error");
        }
    };
        ...
        <>
            ...
            <button
                ...
                className={`w-full py-3 rounded-md font-medium transition-colors ${
                    otp.join('').length === 6 && !isVerifying && !isVerified
