...
//   const getMyCreatedActivities = (userId: string): ActivityField[] => {
//     // อ่านข้อมูลจาก localStorage ทุกครั้ง
//     const stored = JSON.parse(localStorage.getItem("activities") || "[]");
//     console.log("กิจกรรมทั้งหมดใน localStorage:", stored);
//     // กรองเฉพาะกิจกรรมที่สร้างโดย userId นี้
//     return stored.filter((act: ActivityField) => act.create_by === userId);
//   };
...
//     // อ่านข้อมูลจาก localStorage ทุกครั้ง
//     const stored = JSON.parse(localStorage.getItem("activities") || "[]");
//     // กรองเฉพาะกิจกรรมที่มี userId อยู่ใน participants
//     return stored.filter((act: ActivityField) => act.participants?.includes(userId));
//   };

//   // โหลดข้อมูลเมื่อเข้ามาที่หน้านี้ หรือเมื่อ tab เปลี่ยน หรือเมื่อกลับมาจากหน้าอื่น
//   useEffect(() => {
//     console.log("useEffect MyActivity ทำงาน");
//     const token = localStorage.getItem("token");
//     if (!token) {
...
//       const decoded: TokenPayload = jwtDecode(token);
//       const userId = decoded.user_sys_id;
//       console.log("ผู้ใช้ ID:", userId);

//       // เลือกฟังก์ชันดึงข้อมูลตาม activeTab
//       const data =
//         activeTab === "create"
//           ? getMyCreatedActivities(userId)
//           : getMyJoinedActivities(userId);

//       console.log(`ข้อมูลกิจกรรมใน tab ${activeTab}:`, data);
//       setActivities(data);
//     } catch (error) {
//       console.error("เกิดข้อผิดพลาดในการถอดรหัส token:", error);
//     }
//   }, [activeTab, router.asPath]); // เพิ่ม router.asPath เพื่อโหลดข้อมูลใหม่เมื่อกลับมาที่หน้านี้
